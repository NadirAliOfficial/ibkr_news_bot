...
class Engine(threading.Thread):
    ...
    def _on_news(self, item: dict):
        ...
        try:
            ...
            # full_text = f"{title} {teaser} {summary} {body}".strip()  # not used for matching now
            ...
            elif "body" in content and isinstance(content["body"], str) and "NASDAQ:" in content["body"]:
            ...
            # Title-only, whole-word keyword matching
            def _whole_word_matches(text: str, keywords: List[str]) -> List[str]:
                txt = (text or "").lower()
                out = []
                for kw in keywords:
                    k = kw.strip().lower()
            ...
            cfg_keywords = sorted({k.strip() for w in self.cfg.watchlists for k in w.keywords if k.strip()})
            matches = _whole_word_matches(title, cfg_keywords)

            if not matches:
                self.log.info(f"🔸 No keyword match (title-only) for {sym}")
                return
            ...
            if self.cfg.engine.test_mode or not self._ib.isConnected():
                vol = 999999  # bypass in test mode
            ...
            mid = self._trader._mid(sym, self.cfg.engine.test_mode, self.cfg.engine.price_source)
            ...
            if getattr(self, "_loop", None):
                ...
                self.log.error("❌ Cannot place order — IB async loop not found!")
