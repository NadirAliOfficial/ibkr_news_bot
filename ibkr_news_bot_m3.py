...
import os, sys, csv, re, json, time, threading, sqlite3, logging, traceback, requests, ahocorasick
...
class Trader:
    ...
    def _mid(self, symbol: str, test_mode: bool) -> float:
        if test_mode:
            ...
            return 100.00
        try:
            t: Ticker = self.ib.reqTickers(self._stk(symbol))[0]
            if t.bid and t.ask:
...
# -----------------------------
class BZWebSocket:
    def __init__(self, token: str, callback: Callable):
        self.url = f"wss://api.benzinga.com/api/v1/news?token={token}"
...
class Engine(threading.Thread):
    ...
    def _get_first_minute_volume(self, symbol: str, news_time: datetime) -> int:
        """Fetch 1-min bar volume for the news timestamp"""
        try:
            contract = Stock(symbol, "SMART", "USD", primaryExchange="NASDAQ")

            # Align to minute
            start = news_time.replace(second=0, microsecond=0)
    ...
    def _on_news(self, item: dict):
        ...
        try:
            ...
            # Filter by watchlist intersection
            stocks = {s.get("symbol") for s in item.get("stocks", []) if s.get("symbol")}
            related = stocks & self.watch
            if not related:
                return

            self.store.save_news(item, matches, list(related))
            self.log.info(f"MATCH {sorted(related)} {sorted(matches)} | {title}")

            for sym in related:
                # Volume filter
                news_time = datetime.fromisoformat(item.get("created"))
                min_vol = self.cfg.risk.min_news_volume
                vol = self._get_first_minute_volume(sym, news_time)
                if vol < min_vol:
                    self.log.info(f"SKIP {sym}: 1-min volume {vol} < {min_vol}")
                    continue
                ...
                if self.cfg.risk.iceberg and qty > self.cfg.risk.iceberg_child_qty:
                    self._trader.iceberg(sym, qty, mid, tp, sl, reason,
                                         child_qty=self.cfg.risk.iceberg_child_qty)
                else:
    ...
    def run(self):
        try:
            self._ib.connect(self.cfg.ibkr_host,
                             self.cfg.ibkr_port,
        ...
        self.watch = sorted({s for w in self.cfg.watchlists for s in w.tickers})
        ...
        self.running.set()
        self.log.info(f"Engine started. Watchlist={self.watch} Keywords={all_keywords}")
        ...
        while self.running.is_set():
            try:
                ...
                if not now_in_any_window(self.cfg.window_am, self.cfg.window_pm):
                    self.log.info("Outside trading windows; sleeping.")
                ...
                if pnl <= self.cfg.risk.global_loss_usd or pnl >= self.cfg.risk.global_profit_usd:
                    self.log.warning(f"Global PnL cutoff hit ({pnl:.2f}); closing positions.")
                    try:
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("IBKR News Bot — Long-Only (All-in-One)")
        ...
        self.logbox.setStyleSheet("QPlainTextEdit { background: #fafafa; }")
    ...
    def _reload(self):
        ...
        for i, p in enumerate(self.wpages):
    ...
    def _start_engine(self):
        ...
        try:
            # Optionally test IB connection before starting
            test_ib = IB()
