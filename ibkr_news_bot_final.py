...
from PySide6.QtWidgets import (
    ...
    QSpinBox, QDoubleSpinBox, QCheckBox, QGroupBox, QHeaderView, QPushButton, QPlainTextEdit, QComboBox
)
...
class Store:
    def __init__(self, db_path=DB_PATH):
        ...
        try:
            self.conn.execute("ALTER TABLE news ADD COLUMN processed INTEGER DEFAULT 0")
    ...
    def save_news(self, item: dict):
        """Store raw news (no matches yet)."""
        try:
            self.conn.execute("""
                INSERT OR IGNORE INTO news(id, created, title, url, matches, tickers, processed)
                VALUES(?,?,?,?,?,?,0)
            """, (
                item.get("id", int(time.time())),
                item.get("created", datetime.now(EST).isoformat()),
                item.get("title", ""),
                item.get("url", ""),
                "",
                "",
...
# -----------------------------
# Keyword matcher (kept for future; title-only matching below)
# -----------------------------
...
class Trader:
    ...
    def place_bracket(self, symbol: str, qty: int, entry: float, tp: float, sl: float, reason: str) -> Optional[int]:
        ...
        try:
            ...
            time.sleep(0.2)  # slight delay to avoid IBKR pacing
...
class BZWebSocket:
    ...
    def start(self):
        def _on_message(ws, msg):
            try:
                data = json.loads(msg)
                print(f"📰 WS message received at {datetime.now().strftime('%H:%M:%S')}")
                self.callback(data)
            except Exception as e:
...
class Engine(threading.Thread):
    ...
    def _process_pending_news(self):
        """Every few seconds, read stored news, filter and trade."""
        try:
            rows = self.store.conn.execute(
                "SELECT id, title FROM news WHERE processed=0 ORDER BY id ASC LIMIT 20"
            ).fetchall()
            if not rows:
                return
            for nid, title in rows:
                # Keyword match
                matches = self.matcher.match(title.lower())
                if not matches:
                    self.store.conn.execute("UPDATE news SET processed=1 WHERE id=?", (nid,))
                    continue

                # Compute symbol (simple heuristic)
                sym = ""
                m = re.search(r"\b([A-Z]{2,5})\b", title)
                if m:
                    sym = m.group(1).upper()
                if sym and sym in self.watch:
                    self.log.info(f"✅ Matched {sym} | {matches}")
                    self._safe_trade(sym, matches)
                self.store.conn.execute("UPDATE news SET processed=1 WHERE id=?", (nid,))
            self.store.conn.commit()
        except Exception as e:
            self.log.error(f"_process_pending_news error: {e}")
    
    def _safe_trade(self, sym, matches):
        """Run trade logic from main loop safely."""
        try:
            ...
            reason = f"offline:{','.join(matches)}"
            self._trader.place_bracket(sym, qty, mid, tp, sl, reason)
        except Exception as e:
            self.log.error(f"_safe_trade failed: {e}")
    ...
    # Always schedule order placement on the IB asyncio loop thread
    def _place_trade_on_loop(self, sym, mid, tp, sl, qty, reason):
        try:
            ...
            self.log.error(f"_place_trade_on_loop error: {e}")

    def _on_news(self, item: dict):
        """Collector: just store news safely, no IBKR work."""
        try:
            data = item.get("data", {})
            content = data.get("content", {}) or {}
            title = content.get("title", "") or ""
            sym = ""
            if "securities" in content and content["securities"]:
                sym = content["securities"][0].get("symbol", "")
            elif "tickers" in content and content["tickers"]:
                sym = content["tickers"][0]

            sym = (sym or "").replace(":US", "").replace(".N", "").strip().upper()
            self.log.info(f"📰 Collected news: {sym} | {title[:100]}")
            self.store.save_news({
    ...
    def run(self):
        # Create and store the asyncio loop for this thread BEFORE connect()
        try:
...
class SessionsPage(QWidget):
    def __init__(self, cfg: AppCfg, parent=None):
        ...
        grid.addWidget(QLabel("Price source (entry basis)"), 6, 0)
        self.price_src = QComboBox()
        self.price_src.addItems(["bid", "mid", "ask"])
        try:
            idx = ["bid", "mid", "ask"].index(getattr(cfg.engine, "price_source", "mid"))
        except ValueError:
            idx = 1
        self.price_src.setCurrentIndex(idx)
        grid.addWidget(self.price_src, 6, 1)
    ...
    def export_sessions(self, cfg: AppCfg):
        ...
        cfg.engine.price_source = self.price_src.currentText()
        return cfg
